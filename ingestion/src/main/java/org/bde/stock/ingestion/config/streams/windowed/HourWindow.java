...
{
    @Value( "${org.bde.stock.window.hour.length}" )
    private Integer windowLength;

    @Bean
    public KStream<Windowed<String>, AssetCandleAggregator> hourWindowStream( @Qualifier( "candleSource") final KStream<AssetCandleMessageKey, AssetCandleMessageValue> candleSource )
    {
        final var candleSize = Duration.ofMinutes( 60 );
        ...
        final var windowed = candleSource
              .filter( ( key, value ) -> key.getInterval() == 60 )
              ...
              .aggregate( AssetCandleAggregator::new,
                          ...
                          Materialized.<String, AssetCandleAggregator, WindowStore<Bytes, byte[]>>
                                as( "WindowedStoreHour" )
                                .withKeySerde( Serdes.String() )
                                .withValueSerde( aggSerde ) )
              .toStream();

        windowed.to( "asset-candle.window.hour", Produced.with( windowSerde, aggSerde ) );
        windowed.print( Printed.<Windowed<String>, AssetCandleAggregator>toSysOut().withLabel( "Window 60 minute" ) );
        return windowed;
    }
}
